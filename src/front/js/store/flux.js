    ...
    return {
        ...
        actions: {
            ...
            getSingleMovie: async (movieId) => {
                const movie = await fetch(`https://api.themoviedb.org/3/movie/${movieId}`, config);
                const jsonMovie = await movie.json();

                setStore({ film: jsonMovie });
            },
            getSingleTvShow: async (showId) => {
                const tvShow = await fetch(`https://api.themoviedb.org/3/tv/${showId}`, config);
                const jsonTvShow = await tvShow.json();

                setStore({ film: jsonTvShow });
            },
            getMovieCredits: async (movieId) => {
                const credits = await fetch(`https://api.themoviedb.org/3/movie/${movieId}/credits`, config);
                const jsonCredits = await credits.json();

                setStore({ filmCredits: jsonCredits });
            },
            getSeriesCredits: async (serieId) => {
                const credits = await fetch(`https://api.themoviedb.org/3/tv/${serieId}/credits`, config);
                const jsonCredits = await credits.json();

                setStore({ filmCredits: jsonCredits });
            },
            getMovieGenre: async (movieGenre) => {
                const genre = await fetch(`https://api.themoviedb.org/3/${movieGenre}/movie/list?language=en`, config);
            ...
            addRecentlyWatched: async (filmId, filmName, isMovie) => {
                ...
                if (!filmId || !filmName || typeof isMovie === 'undefined') {
                    console.error("Missing required data for adding recently watched film");
                    return;
                }
                ...
                try {
                    const response = await fetch(`${process.env.BACKEND_URL}api/recently_watched`, {
                        method: "POST",
                        headers: {
                            "Content-Type": "application/json",
                        ...
                        body: JSON.stringify({ film_id: filmId, film_name: filmName, is_movie: isMovie }),
                    });
